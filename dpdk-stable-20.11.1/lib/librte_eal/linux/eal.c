...
rte_eal_config_reattach(void)
{
	...
	/* unmap original config  munmap解除先前的映射 */
	...
	config->mem_config = mem_config;//将rte_config.mem_config指向重新映射的内存
...
rte_config_init(void)
{
	...
	case RTE_PROC_SECONDARY:
		if (rte_eal_config_attach() < 0)//打开文件，mmap映射文件到内存
			return -1;
...
// 确认rte_eal_init 是否已经初始化过了
	...
	if (rte_eal_cpu_init() < 0) {//解析 /sys/devices/system/cpux/xx信息  赋值全局结构struct  lcore_config
	...
	if (eal_plugins_init() < 0) {//以so 方式调用插件
	...
	if (eal_trace_init() < 0) {//跟踪库初始化  日志 记录
	...
	if (rte_eal_intr_init() < 0) {// dpdk 线程使用epoll模型， 监听UIO事件，来模拟中断
	 ...
	 * bus through mp channel in the secondary process before the bus scan.
	 dpdk主次进程初始化 生成进程间通信共享文件过滤变量以及进程通信共享文件所在目录
	 根据主次进程的socket 来发消息通信
	 创建线程名为rte_mp_handle 的线程，线程函数为mp_handle
	...
	if (rte_eal_log_init(logid, internal_conf->syslog_facility) < 0) {//调用fopencookie，定义一个定制的写日志接口
	...
	RTE_LCORE_FOREACH_WORKER(i) {
		...
		ret = pthread_create(&lcore_config[i].thread_id, NULL,
				     eal_thread_loop, NULL);//创建子线程，线程执行函数为eal_thread_loop
		if (ret != 0)
			rte_panic("Cannot create thread\n");

		/* Set thread_name for aid in debugging.调用rte_thread_setname给线程设置名称“lcore-slave-xx” */
	 ...
	 * knows they are all ready when this function returns.
	 MASTER lcore调用rte_eal_remote_launch()将回调函数和参数注册到lcore_config[].f和lcore_config[].arg中，
	 并通知SLAVE lcore执行该回调函数
	 检查各个子线程/lcore的状态是否处于WAIT
	 */
	rte_eal_mp_remote_launch(sync_func, NULL, SKIP_MAIN);
	rte_eal_mp_wait_lcore();//等待所有子线程完成工作
